...
pygame.mixer.init()
def play():
    pygame.mixer.music.load("leika_barking.wav")
    pygame.mixer.music.play(loops=0)

def show_log():
    log_window = tk.Toplevel(root)
    log_window.title("Log-Datei")
    log_window.attributes("-topmost",
                          True)  # Set log window to always stay on top
    #log_window.iconbitmap("log.ico") # Funktioniert nicht in Replit
    ...
    def update_log():
        ...
        log_window.after(500, update_log)  #Replit aktualisiert das nicht
...
def open_impressum_window():
    ...
    #impressum_window.iconbitmap("impressum.ico") # Funktioniert nicht in Replit
    ...
    impressum_text.insert(
        tk.END, "Hier kommt die Beschreibung unseres Projektes hin\n\n")
...
def update_files_dropdown():
    default_dir = "./savings"
    json_files = [
        os.path.splitext(file)[0] for file in os.listdir(default_dir)
...
def save_settings():
    settings = {"interval": interval_entry.get(), "length": length_entry.get()}
    default_dir = "./savings"
...
def load_settings():
    filename = filedialog.askopenfilename(initialdir="./savings",
                                          filetypes=[("JSON Files", "*.json")])
    if filename:
...
#root.iconbitmap("rakete.ico") # Funktioniert nicht in Replit

# Title label
title_label = ctk.CTkLabel(root, text="CCCP Space Project", font=("Segoe UI", 20))
...
# Full-screen button
fullscreen_button = ctk.CTkButton(extra_settings_frame,
                               text="⛶",
                               command=toggle_fullscreen)
fullscreen_button.pack(side=tk.RIGHT, padx=10)

# Button to show log
show_log_button = ctk.CTkButton(extra_settings_frame,
                             text="Log Anzeigen",
                             command=show_log)
...
files_dropdown = ttk.Combobox(root,
...
interval_label = ctk.CTkLabel(root, text="Interval:")
interval_label.pack(side=tk.LEFT, padx=(10, 5), pady=(10, 625), anchor=tk.NW)
interval_entry = ctk.CTkEntry(root)
interval_entry.insert(0, "0.1")  # Default interval
interval_entry.pack(side=tk.LEFT, pady=(10, 625))
interval_entry.bind("<KeyRelease>", interval_length_changed)

length_label = ctk.CTkLabel(root, text="Länge:")
length_label.pack(side=tk.LEFT, padx=(10, 5), pady=(10, 625), anchor=tk.NW)
length_entry = ctk.CTkEntry(root)
length_entry.insert(0, "50")  # Default length
length_entry.pack(side=tk.LEFT, pady=(10, 625))
length_entry.bind("<KeyRelease>", interval_length_changed)

# Create a button
button = ctk.CTkButton(root, text="Graphen anzeigen", command=show_graphs)
button.pack()
...
save_button = ctk.CTkButton(footer_frame,
                         text="Einstellungen speichern",
                         command=save_settings)
save_button.pack(side=tk.LEFT, padx=2, pady=5, fill=tk.X, expand=True)

# Impressum button
impressum_button = ctk.CTkButton(footer_frame,
                              text="Impressum",
                              command=open_impressum_window)
...
load_button = ctk.CTkButton(footer_frame,
                         text="Einstellungen abrufen",
